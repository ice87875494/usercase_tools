...
const TABLE_DEFINITIONS = [
  ...
  { id:'gdc0',name:'main-iq-gdc0配置',x:0,y:710,width:480,height:411 },
  { id:'gdc1',name:'main-iq-gdc1配置',x:520,y:710,width:480,height:411 }
];
...
const textFileModules = [
  { name:'imcoverridesettings.txt',endpoint:'/api/imc-overrides',layoutKey:IMC_CODE_LAYOUT_KEY,panel:document.querySelector('#imcCodePanel'),drag:document.querySelector('#imcCodeDrag'),save:document.querySelector('#imcCodeSave'),refresh:document.querySelector('#imcCodeRefresh'),resize:document.querySelector('#imcCodeResize'),path:document.querySelector('#imcCodePath'),code:document.querySelector('#imcCode'),layout:{x:1175,y:80,width:420,height:500,baseX:1175,baseY:80,baseWidth:420,baseHeight:500},dirty:false,sourceLabel:''},
  { name:'Usecase 脚本',endpoint:'/api/usecase-script',layoutKey:SCRIPT_CODE_LAYOUT_KEY,panel:document.querySelector('#scriptCodePanel'),drag:document.querySelector('#scriptCodeDrag'),save:document.querySelector('#scriptCodeSave'),refresh:document.querySelector('#scriptCodeRefresh'),resize:document.querySelector('#scriptCodeResize'),path:document.querySelector('#scriptCodePath'),code:document.querySelector('#scriptCode'),layout:{x:1175,y:620,width:520,height:520,baseX:1175,baseY:620,baseWidth:520,baseHeight:520},dirty:false,sourceLabel:''}
];
const pipelineImageModule = {
  panel:document.querySelector('#pipelineImagePanel'),drag:document.querySelector('#pipelineImageDrag'),refresh:document.querySelector('#pipelineImageRefresh'),resize:document.querySelector('#pipelineImageResize'),path:document.querySelector('#pipelineImagePath'),image:document.querySelector('#pipelineImage'),status:document.querySelector('#pipelineImageStatus'),layout:{x:1175,y:1180,width:900,height:260,baseX:1175,baseY:1180,baseWidth:900,baseHeight:260}
};
const cells = [];
let activeCell = null;
let toastTimer;
...
function initializeTableControls() { const saved=readLayouts();let migrated=false;tableControls.replaceChildren();for(const layout of tableLayouts){const state=saved[layout.id]||{};layout.x=Number.isFinite(state.x)?state.x:layout.baseX;layout.y=Number.isFinite(state.y)?state.y:layout.baseY;const compactSensor=layout.id==='sensor'&&state.compactVersion!==SENSOR_LAYOUT_VERSION;if(compactSensor)migrated=true;layout.width=compactSensor?layout.baseWidth:(Number.isFinite(state.width)?state.width:layout.baseWidth);const legacyResolution=layout.id==='resolution'&&state.height===270;layout.height=compactSensor||legacyResolution?layout.baseHeight:(Number.isFinite(state.height)?state.height:layout.baseHeight);const frame=document.createElement('div');frame.className='table-frame';frame.dataset.tableId=layout.id;const drag=document.createElement('button');drag.type='button';drag.className='table-drag-handle';drag.title=`拖动${layout.name}`;drag.setAttribute('aria-label',`拖动${layout.name}`);const resize=document.createElement('button');resize.type='button';resize.className='table-resize-handle';resize.title=`调整${layout.name}大小`;resize.setAttribute('aria-label',`调整${layout.name}大小`);frame.append(drag,resize);tableControls.append(frame);layout.frame=frame;drag.addEventListener('pointerdown',(event)=>beginLayoutPointer(event,layout,'move'));resize.addEventListener('pointerdown',(event)=>beginLayoutPointer(event,layout,'resize'));renderTableLayout(layout);}if(migrated)saveLayouts(); }
...
async function loadPipelineImage() {
  ...
  try{const response=await fetch('/api/pipeline-diagram-info'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'读取流程图失败');module.path.textContent=`${payload.path} · ${payload.modified_at}`;module.path.title=payload.path;await new Promise((resolve,reject)=>{module.image.onload=resolve;module.image.onerror=()=>reject(new Error('流程图图片加载失败'));module.image.src=`/api/pipeline-diagram.svg?t=${Date.now()}`;});module.status.hidden=true;}
...
function initializePipelineImageModule() {
  ...
  module.refresh.addEventListener('click',(event)=>{event.stopPropagation();loadPipelineImage();});loadPipelineImage();
}
...
async function loadTextFileModule(module,confirmDiscard=true) {
  ...
  try{const response=await fetch(module.endpoint),payload=await response.json();if(!response.ok)throw new Error(payload.error||'读取文件失败');module.sourceLabel=`${payload.path} · ${payload.modified_at}`;module.path.title=payload.path;module.code.textContent=payload.content||'';setTextFileDirty(module,false);}
...
async function saveTextFileModule(module) {
  const content=module.code.innerText.replace(/\r/g,'');module.save.disabled=true;module.save.setAttribute('aria-busy','true');
  try{const response=await fetch(module.endpoint,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({content})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'保存文件失败');module.sourceLabel=`${payload.path} · ${payload.modified_at}`;module.path.title=payload.path;setTextFileDirty(module,false);showToast(`${module.name} 已保存`);}
...
function initializeTextFileModule(module) {
  ...
  for(const control of [module.save,module.refresh,module.code])control.addEventListener('pointerdown',(event)=>event.stopPropagation());
  module.code.addEventListener('input',()=>setTextFileDirty(module,true));module.code.addEventListener('keydown',(event)=>{if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='s'){event.preventDefault();if(module.dirty)saveTextFileModule(module);}});module.save.addEventListener('click',(event)=>{event.stopPropagation();saveTextFileModule(module);});module.refresh.addEventListener('click',(event)=>{event.stopPropagation();loadTextFileModule(module);});loadTextFileModule(module,false);
}
...
async function load() { const response=await fetch('../assets/4k30-normal-preview.svg');if(!response.ok)throw new Error();diagram.innerHTML=await response.text();const svg=diagram.querySelector('svg');svg.removeAttribute('width');svg.removeAttribute('height');addSensorDefinition(svg);tableLayouts=prepareMovableTables(svg);bindCells(svg);initializeTableControls();initializeF2mTrigger();initializeTextFileModules();initializePipelineImageModule();fitToView(); }
function bindTitle() { const saved=localStorage.getItem(TITLE_KEY);if(saved)pageTitle.textContent=saved;document.title=pageTitle.textContent.trim()||'4K30 Normal Preview';pageTitle.addEventListener('input',()=>{const value=pageTitle.textContent.trim();localStorage.setItem(TITLE_KEY,value);document.title=value||'4K30 Normal Preview';saveStatus.textContent='已自动保存';});pageTitle.addEventListener('keydown',(event)=>{if(event.key==='Enter'){event.preventDefault();pageTitle.blur();}}); }
