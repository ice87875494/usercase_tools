...
const normalizeMultiline = (value) => String(value ?? '').replace(/\u00a0/g,' ').replace(/\r/g,'').split('\n').map(line=>normalize(line)).join('\n').trim();
...
function updateFallback(fallback,value) {
  if(!fallback)return;const wrap=Number(fallback.dataset.wrapChars||0),normalizedValue=normalizeMultiline(value),stackPair=fallback.dataset.stackPair==='true'&&!normalizedValue.includes('\n')&&/^\[[^,]+,\s*[^,]+\]$/.test(normalizedValue),explicitLines=(stackPair?normalizedValue.replace(/,\s*/,',\n'):normalizedValue).split('\n'),lines=[];
  for(const explicitLine of explicitLines){
    if(!wrap||explicitLine.length<=wrap){lines.push(explicitLine);continue;}
    const words=explicitLine.split(/(?<=,)|\s+/);let line='';for(const word of words){const next=`${line}${line?' ':''}${word}`.trim();if(line&&next.length>wrap){lines.push(line);line=word;}else line=next;}lines.push(line);
  }
  if(lines.length===1){fallback.textContent=lines[0];return;}
  const centerY=Number(fallback.dataset.centerY||fallback.getAttribute('y')||0),lineHeight=Number(fallback.dataset.lineHeight||fallback.getAttribute('font-size')||11),x=fallback.getAttribute('x');
