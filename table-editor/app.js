...
const PIPELINE_LAYOUT_VERSION = '2026-07-28-expanded';
...
const TABLE_DEFINITIONS = [
  { id:'pipeline',name:'流程图',x:0,y:0,width:1160,height:240,defaultLayout:{x:-3.52104,y:5.67429,width:1325,height:270} },
...
function titleSegments(value) { const text=String(value||''),pattern=/16:9\s+Normal|\d(?=path|\s*scale\b)/ig,segments=[];let cursor=0;for(const match of text.matchAll(pattern)){if(match.index>cursor)segments.push({text:text.slice(cursor,match.index),red:false});segments.push({text:match[0],red:true});cursor=match.index+match[0].length;}if(cursor<text.length)segments.push({text:text.slice(cursor),red:false});return segments; }
function renderPageTitle(value) { const fragment=document.createDocumentFragment();for(const segment of titleSegments(value)){const node=segment.red?document.createElement('span'):document.createTextNode(segment.text);if(segment.red){node.className='title-highlight';node.textContent=segment.text;}fragment.append(node);}pageTitle.replaceChildren(fragment); }
...
function isSubMode() { return /\bsub\b/i.test(pageTitle.textContent); }
function isAinrMode() { return /\bainr\b/i.test(pageTitle.textContent); }
function isRmscMode() { return /\brmsc_off\b/i.test(pageTitle.textContent); }
...
function captureResetBaseline() { resetBaseline={title:pageTitle.textContent.trim(),cells:Object.fromEntries(cells.map(cell=>[keyFor(cell),cell.value])),pipelineNodes:Object.fromEntries(pipelineNodes.map(node=>[node.key,{name:node.value,struck:node.struck}])),tableLayouts:Object.fromEntries(tableLayouts.map(layout=>[layout.id,copyLayout(layout)])),f2mLayout:{x:f2mTriggerLayout.x,y:f2mTriggerLayout.y},textModules:textFileModules.map(module=>({layout:copyLayout(module.layout),name:module.currentName,title:module.title.textContent,content:module.code.textContent,sourceLabel:module.sourceLabel})),pipelineImage:{layout:copyLayout(pipelineImageModule.layout),name:pipelineImageModule.currentName,title:pipelineImageModule.title.textContent,sourceLabel:pipelineImageModule.sourceLabel,src:pipelineImageModule.image.getAttribute('src')||''}}; }
...
function drawExportTitle(context,value,x,y) { for(const segment of titleSegments(value)){context.fillStyle=segment.red?'#d60000':'#26313a';context.fillText(segment.text,x,y);x+=context.measureText(segment.text).width;} }
...
async function renderExportCanvas() {
  ...
  try{
    ...
    try{
      ...
      context.font='600 18px "Segoe UI","Microsoft YaHei",Arial,sans-serif';drawExportTitle(context,pageTitle.textContent.trim()||DEFAULT_TITLE,20,27);context.fillStyle='#7a828c';context.font='11px "Segoe UI","Microsoft YaHei",Arial,sans-serif';context.fillText('4K30 配置总览',20,44);context.fillStyle='#d9dee4';context.fillRect(0,headerHeight-1,exported.bounds.width,1);
...
async function exportPng() {
  try{const canvas=await renderExportCanvas(),png=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!png)throw new Error('无法生成 PNG');const response=await fetch('/api/export-image',{method:'POST',headers:{'Content-Type':'image/png','X-Export-Title':pageTitle.textContent.trim()||DEFAULT_TITLE},body:png}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'图片导出失败');showToast(`已写入 output/${payload.name}`);}
...
function bindTitle() { const saved=localStorage.getItem(TITLE_KEY),legacyDefaults=new Set(['4K30 Normal Preview','4K30 16:9 Normal 2path Multi-scale Preview','4K30 16:9 Normal 2path Multi-scale\u00a0Preview']),migrated=migrateLegacyTitle(saved),value=saved&&!legacyDefaults.has(saved)?migrated:DEFAULT_TITLE;renderPageTitle(value||DEFAULT_TITLE);if(saved!==pageTitle.textContent)localStorage.setItem(TITLE_KEY,pageTitle.textContent);document.title=pageTitle.textContent.trim()||DEFAULT_TITLE;pageTitle.addEventListener('input',()=>{const next=pageTitle.textContent.trim();localStorage.setItem(TITLE_KEY,next);document.title=next||DEFAULT_TITLE;if(cells.length)calculateResolutionRules();if(pipelineNodes.length)applyPipelineModes();saveStatus.textContent='已自动保存';});pageTitle.addEventListener('keydown',(event)=>{if(event.key==='Enter'){event.preventDefault();renderPageTitle(normalizeTitleModes(pageTitle.textContent.trim())||DEFAULT_TITLE);pageTitle.dispatchEvent(new InputEvent('input',{bubbles:true}));pageTitle.blur();}});pageTitle.addEventListener('blur',()=>renderPageTitle(pageTitle.textContent.trim()||DEFAULT_TITLE)); }
