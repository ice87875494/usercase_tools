...
const PIPELINE_LAYOUT_VERSION = '2026-07-28-original';
...
const STATE_VERSION = '2026-07-28-v2';
...
const SUB_PIPELINE_NODE_KEYS = new Set(['50:74','160:74','270:74','380:74','470:24','540:24','645:24','740:24','825:24']);
...
let ainrPublicPipeline=null;
let rmscPublicPipeline=null;
let resetBaseline=null;

const normalize = (value) => String(value ?? '').replace(/\u00a0/g, ' ').replace(/\s+/g, ' ').trim();
const normalizeMultiline = (value) => String(value ?? '').replace(/\u00a0/g,' ').replace(/\r/g,'').trim();
function titleSegments(value) { const text=String(value||''),segments=[];let cursor=0;for(const match of text.matchAll(/\d+/g)){if(match.index>cursor)segments.push({text:text.slice(cursor,match.index),red:false});segments.push({text:match[0],red:true});cursor=match.index+match[0].length;}if(cursor<text.length)segments.push({text:text.slice(cursor),red:false});return segments; }
function pageTitleValue() { return [...pageTitle.childNodes].map(node=>node instanceof HTMLSelectElement?node.value:node.textContent).join(''); }
function appendTitleText(fragment,value) { for(const segment of titleSegments(value)){const node=segment.red?document.createElement('span'):document.createTextNode(segment.text);if(segment.red){node.className='title-highlight';node.textContent=segment.text;}fragment.append(node);} }
function renderPageTitle(value) { const text=String(value||DEFAULT_TITLE).replace(/\brmsc_off\b/gi,'rmsc'),match=text.match(/\b(Normal|Ainr|Sub|rmsc)\b/i),fragment=document.createDocumentFragment();if(!match){appendTitleText(fragment,text);pageTitle.replaceChildren(fragment);return;}appendTitleText(fragment,text.slice(0,match.index));const select=document.createElement('select');select.className='title-mode-select';select.contentEditable='false';select.setAttribute('aria-label','选择公共链路模式');for(const optionName of ['Normal','Ainr','Sub','rmsc']){const option=document.createElement('option');option.value=optionName;option.textContent=optionName;select.append(option);}select.value=match[1].toLowerCase()==='ainr'?'Ainr':match[1].toLowerCase()==='sub'?'Sub':match[1].toLowerCase()==='rmsc'?'rmsc':'Normal';select.addEventListener('change',()=>{const next=pageTitleValue();localStorage.setItem(TITLE_KEY,next);document.title=next||DEFAULT_TITLE;if(cells.length)calculateResolutionRules();if(pipelineNodes.length)applyPipelineModes();saveStatus.textContent='已自动保存';renderPageTitle(next);});fragment.append(select);appendTitleText(fragment,text.slice(match.index+match[0].length));pageTitle.replaceChildren(fragment); }
...
function titlePathScales() { const modes={1:null,2:null};for(const match of pageTitleValue().matchAll(/([12])\s*path\s*([123])\s*scale/ig))modes[Number(match[1])]=Number(match[2]);if(modes[1]===null&&modes[2]===null)return{1:3,2:3};return modes; }
function isSubMode() { return /\bsub\b/i.test(pageTitleValue()); }
function isAinrMode() { return /\bainr\b/i.test(pageTitleValue()); }
function isRmscMode() { return /\brmsc\b/i.test(pageTitleValue()); }
function isSubPipelineConnector(path) { const d=path.getAttribute('d')||'',match=d.match(/^M\s+([\d.]+)\s+([\d.]+)/);if(!match)return false;return Number(match[1])<400||/\s(?:16|20|24)(?:\s|$)/.test(d)||d==='M 420 70 L 430 70'; }
function applyAinrPublicPipeline() { if(!ainrPublicPipeline)return;const ainrActive=isAinrMode(),rmscActive=isRmscMode();ainrPublicPipeline.overlay.style.display=ainrActive?'':'none';if(rmscPublicPipeline)rmscPublicPipeline.overlay.style.display=rmscActive?'':'none';for(const element of ainrPublicPipeline.sourceElements)element.style.display=ainrActive||rmscActive?'none':''; }
function applyPipelineModes() { applyAinrPublicPipeline();const subMode=isSubMode(),scales=titlePathScales();for(const node of pipelineNodes){const rule=PIPELINE_NODE_MODE_RULES.get(node.key),disabled=subMode?!SUB_PIPELINE_NODE_KEYS.has(node.key):Boolean(rule)&&((scales[rule.path]??0)<rule.minScale);setPipelineNodeDisabledState(node,disabled);}for(const connector of pipelineConnectors){const disabled=subMode?!connector.subEnabled:(scales[connector.path]??0)<connector.minScale;setPipelineConnectorDisabledState(connector,disabled);} }
function addAinrNode(group,{x,y=50,label,lines,fill,stroke,mode='ainr'}) { const ns='http://www.w3.org/2000/svg',node=document.createElementNS(ns,'g'),rect=document.createElementNS(ns,'rect'),text=document.createElementNS(ns,'text');rect.setAttribute('x',String(x));rect.setAttribute('y',String(y));rect.setAttribute('width','70');rect.setAttribute('height','40');rect.setAttribute('rx','8');rect.setAttribute('ry','8');rect.setAttribute('fill',fill);rect.setAttribute('stroke',stroke);rect.setAttribute('stroke-width','2');node.append(rect);text.setAttribute('x',String(x+35));text.setAttribute('fill','#111111');text.setAttribute('font-family','Helvetica');text.setAttribute('font-size',lines.length>1?'12px':'13px');text.setAttribute('text-anchor','middle');lines.forEach((line,index)=>{const span=document.createElementNS(ns,'tspan');span.setAttribute('x',String(x+35));span.setAttribute('y',String(lines.length>1?y+21+index*14:y+27));span.textContent=line;text.append(span);});node.append(text);node.dataset[mode==='rmsc'?'rmscPublicNode':'ainrPublicNode']=label;group.append(node); }
function addAinrConnector(group,start,end,y=70) { const ns='http://www.w3.org/2000/svg',line=document.createElementNS(ns,'path'),arrow=document.createElementNS(ns,'path');line.setAttribute('d',`M ${start} ${y} L ${end-10} ${y}`);line.setAttribute('fill','none');line.setAttribute('stroke','#000000');line.setAttribute('stroke-width','2');arrow.setAttribute('d',`M ${end-12} ${y-4} L ${end-4} ${y} L ${end-12} ${y+4}`);arrow.setAttribute('fill','none');arrow.setAttribute('stroke','#000000');arrow.setAttribute('stroke-width','2');group.append(line,arrow); }
function setupAinrPublicPipeline(svg) { const group=svg.querySelector('[data-table-content="pipeline"]');if(!group||group.querySelector('[data-ainr-public-pipeline]'))return;const sourceElements=[],sourceLabels=new Set(['Sensor RAW','FPP','SCPP','VDPP']);for(const foreignObject of group.querySelectorAll('foreignObject')){const editor=foreignObject.querySelector('div > div > div');if(!editor||!sourceLabels.has(normalize(editor.innerText)))continue;sourceElements.push(foreignObject);const fallback=foreignObject.closest('switch')?.querySelector('text');if(fallback)sourceElements.push(fallback);}for(const rect of group.querySelectorAll('rect')){if(['10','120','230','340'].includes(rect.getAttribute('x'))&&rect.getAttribute('y')==='50')sourceElements.push(rect);}for(const path of group.querySelectorAll('path')){const d=path.getAttribute('d')||'';if(/^(?:M (?:10|120|230|340) 50|M (?:90|109\\.76|200|219\\.76|310|329\\.76) (?:70|74))/.test(d))sourceElements.push(path);}const overlay=document.createElementNS('http://www.w3.org/2000/svg','g');overlay.dataset.ainrPublicPipeline='';const nodes=[{x:10,label:'Sensor RAW',lines:['Sensor','RAW'],fill:'#dae8fc',stroke:'#6c8ebf'},{x:95,label:'FPP',lines:['FPP'],fill:'#dae8fc',stroke:'#6c8ebf'},{x:180,label:'SCPP',lines:['SCPP'],fill:'#fff2cc',stroke:'#d6b656'},{x:265,label:'RAWPP (AINR)',lines:['RAWPP','(AINR)'],fill:'#f8cecc',stroke:'#b85450'},{x:350,label:'VDPP',lines:['VDPP'],fill:'#fff2cc',stroke:'#d6b656'}];for(let index=0;index<nodes.length-1;index++)addAinrConnector(overlay,nodes[index].x+70,nodes[index+1].x);for(const node of nodes)addAinrNode(overlay,node);group.append(overlay);ainrPublicPipeline={overlay,sourceElements:[...new Set(sourceElements)]}; }
function setupRmscPublicPipeline(svg) { const group=svg.querySelector('[data-table-content="pipeline"]');if(!group||group.querySelector('[data-rmsc-public-pipeline]'))return;const ns='http://www.w3.org/2000/svg',overlay=document.createElementNS(ns,'g'),nodes=[{x:10,y:50,label:'Sensor RAW',lines:['Sensor','RAW'],fill:'#dae8fc',stroke:'#6c8ebf',mode:'rmsc'},{x:95,y:50,label:'FPP',lines:['FPP'],fill:'#dae8fc',stroke:'#6c8ebf',mode:'rmsc'},{x:180,y:50,label:'RAWPP (RMSC)',lines:['RAWPP','(RMSC)'],fill:'#f8cecc',stroke:'#b85450',mode:'rmsc'},{x:265,y:50,label:'VDPP',lines:['VDPP'],fill:'#fff2cc',stroke:'#d6b656',mode:'rmsc'},{x:350,y:50,label:'VEPPmain',lines:['VEPPmain'],fill:'#d5e8d4',stroke:'#82b366',mode:'rmsc'},{x:95,y:130,label:'FPP (rbin)',lines:['FPP','(rbin)'],fill:'#dae8fc',stroke:'#6c8ebf',mode:'rmsc'},{x:180,y:130,label:'SCPP 3ast only',lines:['SCPP','3ast only'],fill:'#fff2cc',stroke:'#d6b656',mode:'rmsc'}];overlay.dataset.rmscPublicPipeline='';for(let index=0;index<4;index++)addAinrConnector(overlay,nodes[index].x+70,nodes[index+1].x,70);addAinrConnector(overlay,165,180,150);const split=document.createElementNS(ns,'path'),splitArrow=document.createElementNS(ns,'path'),caption=document.createElementNS(ns,'text');split.setAttribute('d','M 80 70 L 84 70 L 84 150 L 85 150');split.setAttribute('fill','none');split.setAttribute('stroke','#000000');split.setAttribute('stroke-width','2');splitArrow.setAttribute('d','M 83 146 L 91 150 L 83 154');splitArrow.setAttribute('fill','none');splitArrow.setAttribute('stroke','#000000');splitArrow.setAttribute('stroke-width','2');caption.setAttribute('x','45');caption.setAttribute('y','112');caption.setAttribute('fill','#111111');caption.setAttribute('font-family','Helvetica');caption.setAttribute('font-size','12px');caption.setAttribute('text-anchor','middle');caption.textContent='quadbayer';overlay.append(split,splitArrow,caption);for(const node of nodes)addAinrNode(overlay,node);group.append(overlay);rmscPublicPipeline={overlay}; }
...
function bindPipelineNodes(svg) {
  ...
  const trackedPaths=new Set(pipelineConnectors.flatMap(connector=>connector.paths));
  for(const path of group.querySelectorAll('path')){if(trackedPaths.has(path))continue;pipelineConnectors.push({paths:[path],base:[{stroke:path.getAttribute('stroke'),'stroke-dasharray':path.getAttribute('stroke-dasharray'),style:path.getAttribute('style')}],subEnabled:isSubPipelineConnector(path)});}
...
function titleAspectRatio() { const match=pageTitleValue().match(/(\d+(?:\.\d+)?)\s*[:：]\s*(\d+(?:\.\d+)?)/);if(!match)return null;const width=Number(match[1]),height=Number(match[2]);return width>0&&height>0?width/height:null; }
...
function saveLayouts() { localStorage.setItem(LAYOUT_KEY,JSON.stringify(Object.fromEntries(tableLayouts.map(layout=>[layout.id,{x:layout.x,y:layout.y,width:layout.width,height:layout.height,...(layout.id==='pipeline'?{pipelineVersion:PIPELINE_LAYOUT_VERSION}:{}),...(layout.id==='sensor'?{compactVersion:SENSOR_LAYOUT_VERSION}:{}),...(layout.id.includes('gdc')?{headerVersion:GDC_HEADER_LAYOUT_VERSION}:{})}])))); }
...
function initializeTableControls() {
  ...
  for(const layout of tableLayouts){
    const state=saved[layout.id]||{},isGdc=layout.id==='gdc0'||layout.id==='gdc1',expandedPipeline=layout.id==='pipeline'&&state.pipelineVersion!==PIPELINE_LAYOUT_VERSION,legacyGdc=isGdc&&Number.isFinite(state.y)&&state.headerVersion!==GDC_HEADER_LAYOUT_VERSION,legacyScale=legacyGdc&&Number.isFinite(state.height)?state.height/411:1;
    layout.x=expandedPipeline?layout.defaultX:(Number.isFinite(state.x)?state.x:layout.defaultX);
    layout.y=expandedPipeline?layout.defaultY:(legacyGdc?state.y-30*legacyScale:(Number.isFinite(state.y)?state.y:layout.defaultY));
    const compactSensor=layout.id==='sensor'&&state.compactVersion!==SENSOR_LAYOUT_VERSION;if(compactSensor||legacyGdc||expandedPipeline)migrated=true;
    layout.width=expandedPipeline?layout.defaultWidth:(compactSensor?layout.defaultWidth:(Number.isFinite(state.width)?state.width:layout.defaultWidth));
    const legacyResolution=layout.id==='resolution'&&state.height===270;
    layout.height=expandedPipeline?layout.defaultHeight:(legacyGdc?(Number.isFinite(state.height)?state.height+30*legacyScale:layout.defaultHeight):(compactSensor||legacyResolution?layout.defaultHeight:(Number.isFinite(state.height)?state.height:layout.defaultHeight)));
...
function configureDimensionCell(cell) {
  ...
  if(cell.fallback){cell.fallback.setAttribute('fill',emphasized?'#d60000':'#111111');cell.fallback.setAttribute('font-weight',emphasized?'700':'normal');}
}
...
function copyLayout(layout) { return {x:layout.x,y:layout.y,width:layout.width,height:layout.height}; }
function captureResetBaseline() { const baseLayout=layout=>({x:layout.baseX,y:layout.baseY,width:layout.baseWidth,height:layout.baseHeight});resetBaseline={title:DEFAULT_TITLE,cells:Object.fromEntries(cells.map(cell=>[keyFor(cell),cell.defaultValue])),pipelineNodes:Object.fromEntries(pipelineNodes.map(node=>[node.key,{name:node.defaultValue,struck:node.defaultStruck}])),tableLayouts:Object.fromEntries(tableLayouts.map(layout=>[layout.id,{x:layout.defaultX,y:layout.defaultY,width:layout.defaultWidth,height:layout.defaultHeight}])),f2mLayout:{x:f2mTriggerLayout.baseX,y:f2mTriggerLayout.baseY},textModules:textFileModules.map(module=>({layout:baseLayout(module.layout),name:module.currentName,title:module.title.textContent,content:module.code.textContent,sourceLabel:module.sourceLabel})),pipelineImage:{layout:baseLayout(pipelineImageModule.layout),name:pipelineImageModule.currentName,title:pipelineImageModule.title.textContent,sourceLabel:pipelineImageModule.sourceLabel,src:pipelineImageModule.image.getAttribute('src')||''}}; }
function resetTable() { if(!resetBaseline){showToast('页面内容尚未加载完成');return;}renderPageTitle(resetBaseline.title);localStorage.setItem(TITLE_KEY,resetBaseline.title);document.title=resetBaseline.title;for(const cell of cells){updateCell(cell,resetBaseline.cells[keyFor(cell)]??cell.value,false);if(cell.config)normalizeConfiguredCell(cell);}for(const node of pipelineNodes){const saved=resetBaseline.pipelineNodes[node.key];if(!saved)continue;node.value=saved.name;node.struck=saved.struck;renderPipelineNode(node,true);}setActivePipelineNode(null);for(const layout of tableLayouts){const saved=resetBaseline.tableLayouts[layout.id];if(!saved)continue;Object.assign(layout,saved);renderTableLayout(layout);}Object.assign(f2mTriggerLayout,resetBaseline.f2mLayout);renderF2mTriggerLayout();for(const [index,module] of textFileModules.entries()){const saved=resetBaseline.textModules[index];if(!saved)continue;Object.assign(module.layout,saved.layout);module.currentName=saved.name;module.title.textContent=saved.title;module.code.textContent=saved.content;module.sourceLabel=saved.sourceLabel;renderTextFileLayout(module);setTextFileDirty(module,false);}const imageBaseline=resetBaseline.pipelineImage;Object.assign(pipelineImageModule.layout,imageBaseline.layout);pipelineImageModule.currentName=imageBaseline.name;pipelineImageModule.title.textContent=imageBaseline.title;pipelineImageModule.sourceLabel=imageBaseline.sourceLabel;pipelineImageModule.image.src=imageBaseline.src;renderPipelineImageLayout();setPipelineImageDirty(false);calculateResolutionRules(false);applyPipelineModes();saveDraft();savePipelineNodeDraft();saveLayouts();saveF2mTriggerLayout();for(const module of textFileModules)saveTextFileLayout(module);savePipelineImageLayout();showToast('已恢复当前网页加载时的内容'); }
...
function drawExportTitle(context,value,x,y) { for(const segment of titleSegments(value)){context.font=segment.red?'700 18px "Segoe UI","Microsoft YaHei",Arial,sans-serif':'600 18px "Segoe UI","Microsoft YaHei",Arial,sans-serif';context.fillStyle=segment.red?'#d60000':'#26313a';context.fillText(segment.text,x,y);x+=context.measureText(segment.text).width;} }
...
async function renderExportCanvas() {
  ...
  try{
    ...
    try{
      ...
      context.font='600 18px "Segoe UI","Microsoft YaHei",Arial,sans-serif';drawExportTitle(context,pageTitleValue().trim()||DEFAULT_TITLE,20,27);context.fillStyle='#7a828c';context.font='11px "Segoe UI","Microsoft YaHei",Arial,sans-serif';context.fillText('4K30 配置总览',20,44);context.fillStyle='#d9dee4';context.fillRect(0,headerHeight-1,exported.bounds.width,1);
...
async function exportPng() {
  try{const canvas=await renderExportCanvas(),png=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!png)throw new Error('无法生成 PNG');const response=await fetch('/api/export-image',{method:'POST',headers:{'Content-Type':'image/png','X-Export-Title':pageTitleValue().trim()||DEFAULT_TITLE},body:png}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'图片导出失败');showToast(`已写入 output/${payload.name}`);}
  catch(error){console.error(error);showToast('图片导出失败');}
}
async function load() { if(location.protocol==='file:')throw new Error('FILE_PROTOCOL');const response=await fetch('../assets/4k30-normal-preview.svg');if(!response.ok)throw new Error();diagram.innerHTML=await response.text();const svg=diagram.querySelector('svg');svg.removeAttribute('width');svg.removeAttribute('height');svg.setAttribute('viewBox',`-.5 -.5 ${SIZE.width} ${SIZE.height}`);addSensorDefinition(svg);tableLayouts=prepareMovableTables(svg);setupAinrPublicPipeline(svg);setupRmscPublicPipeline(svg);bindPipelineNodes(svg);bindCells(svg);initializeTableControls();initializeF2mTrigger();const resourceLoads=Promise.all([initializeTextFileModules(),initializePipelineImageModule()]);fitToView();await resourceLoads;captureResetBaseline(); }
...
function bindTitle() { const saved=localStorage.getItem(TITLE_KEY),legacyDefaults=new Set(['4K30 Normal Preview','4K30 16:9 Normal 2path Multi-scale Preview','4K30 16:9 Normal 2path Multi-scale\u00a0Preview']),migrated=migrateLegacyTitle(saved),value=saved&&!legacyDefaults.has(saved)?migrated:DEFAULT_TITLE;renderPageTitle(value||DEFAULT_TITLE);if(saved!==pageTitleValue())localStorage.setItem(TITLE_KEY,pageTitleValue());document.title=pageTitleValue().trim()||DEFAULT_TITLE;pageTitle.addEventListener('input',()=>{const next=pageTitleValue().trim();localStorage.setItem(TITLE_KEY,next);document.title=next||DEFAULT_TITLE;if(cells.length)calculateResolutionRules();if(pipelineNodes.length)applyPipelineModes();saveStatus.textContent='已自动保存';});pageTitle.addEventListener('keydown',(event)=>{if(event.key==='Enter'){event.preventDefault();const next=normalizeTitleModes(pageTitleValue().trim())||DEFAULT_TITLE;renderPageTitle(next);localStorage.setItem(TITLE_KEY,next);document.title=next;if(cells.length)calculateResolutionRules();if(pipelineNodes.length)applyPipelineModes();pageTitle.blur();}});pageTitle.addEventListener('blur',(event)=>{if(event.relatedTarget?.classList.contains('title-mode-select'))return;const next=normalizeTitleModes(pageTitleValue().trim())||DEFAULT_TITLE;renderPageTitle(next);localStorage.setItem(TITLE_KEY,next);document.title=next;}); }
...
document.addEventListener('click',(event)=>{if(!fieldMenu.contains(event.target))closeMenu();});pipelineStrikeButton.addEventListener('click',togglePipelineNodeStrike);document.querySelector('#zoomIn').addEventListener('click',()=>zoomCenter(1.2));document.querySelector('#zoomOut').addEventListener('click',()=>zoomCenter(1/1.2));document.querySelector('#fitView').addEventListener('click',fitToView);document.querySelector('#fullscreen').addEventListener('click',async()=>document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen());document.querySelector('#exportPng').addEventListener('click',exportPng);document.querySelector('#resetTable').addEventListener('click',resetTable);window.addEventListener('resize',fitToView);bindTitle();setTimeout(()=>document.querySelector('.canvas-hint').style.opacity='0',4200);load().catch((error)=>{if(error.message==='FILE_PROTOCOL'){diagram.innerHTML='<div class="launch-notice"><strong>需要通过本地服务打开</strong><span>请双击上一级目录中的 start-table-editor.cmd</span></div>';return;}showToast('图表加载失败');});
