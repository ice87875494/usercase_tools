...
“复位”会恢复表格值、流程模块文字/横线和各模块布局的本地默认状态，不会改写外部 TXT、SH 或 SVG 文件。
...
```text
4K30 16:9 Normal 1path3scale 2path3scale preview
```
...
浏览器本地存储保存表格值、标题、流程模块名称/删除线以及各模块布局。清除浏览器站点数据或使用“复位”会影响这些本地状态。
